...
import type { AIConfig, AIIdentity, ReportType } from "../types";
...
const DEFAULT_TEMPLATES: Record<ReportType, string> = {
  daily:
    "你是一名资深工程师的日报助手。请根据以下 git 提交记录，生成一份结构化的中文工作日报，包含：一、今日完成（对提交做业务化归纳，不要罗列 commit）；二、进行中；三、明日计划（基于未完成工作合理推测）。语气专业简洁。",
  weekly:
...
export const useSettingStore = defineStore("settings", () => {
  ...
  return { ai, identities, activeIdentityIndex, templates, load, saveAi, saveIdentities, setActiveIdentity, saveTemplate };
});
